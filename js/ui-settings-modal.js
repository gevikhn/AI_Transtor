  ...
  if (svc.maxTokens!==undefined && svc.maxTokens!=='') svc.maxTokens = Number(svc.maxTokens); else svc.maxTokens = undefined;
    ...
    if (apiInput){
      if (apiInput.dataset.changed==='1'){
        ...
        else if (cfg.masterPasswordEnc){ try { mpPlain = await decryptMasterPassword(cfg.masterPasswordEnc); } catch { mpPlain=''; } }
        try { svc.apiKeyEnc = await encryptApiKey(raw, mpPlain, svc.id); }
    ...
    if (mp){
      if (mp.dataset.changed==='1'){
        ...
        if (plainMp){
          ...
          if (apiInput && apiInput.dataset.changed!=='1'){
            try {
              const rawKey = unlockedPlainKey || (cfg.masterPasswordEnc ? await decryptApiKey(svc.apiKeyEnc, await decryptMasterPassword(cfg.masterPasswordEnc), svc.id) : svc.apiKeyEnc);
              svc.apiKeyEnc = await encryptApiKey(rawKey, plainMp, svc.id);
          ...
          if (apiInput && apiInput.dataset.changed!=='1'){
            try {
              const rawKey = unlockedPlainKey || (cfg.masterPasswordEnc ? await decryptApiKey(svc.apiKeyEnc, await decryptMasterPassword(cfg.masterPasswordEnc), svc.id) : svc.apiKeyEnc);
              svc.apiKeyEnc = rawKey;
            } catch { statusEl.textContent='主密码清除失败'; return; }
          }
        }
      } else {
