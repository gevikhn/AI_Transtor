...
默认建议快捷键为 `Alt+T`，用于触发扩展图标并打开侧边栏；如果当前网页有选中文本，会自动按“翻译选中文本”流程载入并翻译。Chrome 只会在安装时尝试应用 `suggested_key`；如果扩展已加载过、用户曾清空过快捷键，或开发模式重载保留了旧状态，快捷键页可能仍显示“未设置”。请在 `chrome://extensions/shortcuts` 中手动设置、修改或关闭。扩展设置页会显示当前已分配的快捷键，若为空则表示未分配或已关闭。
